class SanPham:
    ...
    def xuat_thong_tin(self):
        print(f"Tên sản phẩm: {self.__tensp} - sanpham.py:34")
        print(f"Giá sản phẩm: {self.__gia} - sanpham.py:35")
        print(f"Giảm giá: {self.__giam_gia}% - sanpham.py:36")
        print(f"Thuế nhập khẩu: {self.thue_nhap_khau()} - sanpham.py:37")
    ...
    def thue_nhap_khau(self):
        return self.gia * 0.1
    def xuat_thong_tin(self):
        print(f"sản phẩm {self.ten_san_pham} có giá {self.gia} được giảm giá hết {self.giam_gia} và thuế nhập khẩu {self.thue_nhap_khau()} - sanpham.py:51")
    def _str_(self):
