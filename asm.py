class NV:
    ...
    def xuat(self):
        print(f"NV,{self.get_ma_nv()},{self.get_ho_ten()},{self.get_luong_co_ban()},{self.get_thu_nhap()} - asm.py:25")
...
class TT(NV):
    ...
    def xuat(self):
        print(f"TT,{self.get_ma_nv()},{self.get_ho_ten()},{self.get_luong_co_ban()},{self.__hoa_hong},{self.__doanh_so},{self.get_thu_nhap()} - asm.py:35")
...
class TP(NV):
    ...
    def xuat(self):
        print(f"TP,{self.get_ma_nv()},{self.get_ho_ten()},{self.get_luong_co_ban()},{self.__luong_trach_nhiem},{self.get_thu_nhap()} - asm.py:44")

# Danh sách nhân viên
danh_sach_nv = []

def cn1():
    print("Chức năng 1: Nhập danh sách nhân viên từ bàn phím. Lưu thông tin nhân viên vào file. - asm.py:50")
    so_luong_nv = int(input("Nhập số lượng nhân viên cần lưu:"))
    for i in range(so_luong_nv):
        print(f"Nhập thông tin nhân viên thứ {i+1}: - asm.py:53")
        ...
        if vitri == "NV":
            ...
            print("Vị trí nhân viên không hợp lệ. Vui lòng nhập lại. - asm.py:69")
            continue
        danh_sach_nv.append(nv)
    print("Đã nhập xong danh sách nhân viên: - asm.py:72")
...
def cn2():
    print("Chức năng 2: Đọc thông tin nhân viên từ file và xuất danh sách nhân viên - asm.py:86")
    pass
def cn3():
    print("Chức năng 3: Tìm và hiển thị nhân viên theo mã nhập từ bàn phím. - asm.py:89")
    ma = input("Nhập mã nhân viên cần tìm:")
    for nv in danh_sach_nv:
        if nv.get_ma_nv() == ma:
            print("Thông tin nhân viên: - asm.py:93")
            nv.xuat()
            return
    print("Không tìm thấy nhân viên với mã đã nhập. - asm.py:96")

def cn4():
    print("Chức năng 4: Xóa nhân viên theo mã nhập từ bàn phím. Cập nhật file dữ liệu. - asm.py:99")
    pass
def cn5():
    print("Chức năng 5: Cập nhật thông tin nhân viên theo mã nhập từ bàn phím và ghi thay đổi vào file. - asm.py:102")
    pass
def cn6():
    print("Chức năng 6: Tìm các nhân viên theo khoảng lương nhập từ bàn phím. - asm.py:105")
    luong_min = float(input("Nhập mức lương tối thiểu: "))
    luong_max = float(input("Nhập mức lương tối đa: "))
    print(f"Các nhân viên có khoảng lương: {luong_min}  {luong_max} - asm.py:108")
    for nv in danh_sach_nv:
        if luong_min <= nv.get_thu_nhap() <= luong_max:
            nv.xuat()

def cn7():
    print("Chức năng 7: Sắp xếp nhân viên theo họ và tên. - asm.py:114")
    danh_sach_nv.sort(key=lambda nv: nv.get_ho_ten())
    for nv in danh_sach_nv:
        nv.xuat()

def cn8():
    print("Chức năng 8: Sắp xếp nhân viên theo thu nhập. - asm.py:120")
    danh_sach_nv.sort(key=lambda nv: nv.get_thu_nhap(), reverse=True)
    for nv in danh_sach_nv:
        nv.xuat()

def cn9():
    print("Chức năng 9: Xuất 5 nhân viên có thu nhập cao nhất. - asm.py:126")
    danh_sach_nv.sort(key=lambda nv: nv.get_thu_nhap(), reverse=True)
...
def cn10():
    print("Chức năng 10: Thoát chương trình. - asm.py:133")
    print("Kết thúc chương trình. - asm.py:134")
